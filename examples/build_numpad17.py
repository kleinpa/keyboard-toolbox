...
def main(argv):
    ...
    def pitch_offset(pose, x, y, pitch=19):
        return Position(x=pose.x + x * pitch, y=pose.y - y * pitch)

    kb.hole_positions.extend([
        pitch_offset(kb.keys[0].pose, 1, -1),
        pitch_offset(kb.keys[2].pose, 1, -1),
        pitch_offset(kb.keys[8].pose, 1, -1),
        pitch_offset(kb.keys[12].pose, 1, -1),
        pitch_offset(kb.keys[10].pose, 1, -1),
    ])
    ...
    kb.controller_pose.CopyFrom(between_pose(kb.keys[5].pose,
                                             kb.keys[10].pose))
    kb.controller_pose.y += 2
    kb.info_pose.CopyFrom(
        project_to_outline(outline,
